...
type ImageGalleryProps = React.InputHTMLAttributes<HTMLInputElement> & {
  ...
  paramsSetter: (url: string | undefined) => void;
  clickAction: () => void;
  size: number;
  numImages: number;
};
...
function ImageSelect(props: ImageItemProps) {
  ...
  const bordercolor = isHover ? "#f7a319" : "black";
  const bgalpha = isHover ? 0.05 : 0.6;
  const textalpha = isHover ? 1.0 : 0.0;
  const label = isHover ? "Connect" : "X";
  const style = {
    width: props.size,
    height: props.size,
  ...
  return (
    <button
      ...
      onClick={async () => {
        await props.paramsSetter(props.imageUrl);
        props.clickAction();
      }}
    >
      {" "}
...
export const ImageGallery = (props: ImageGalleryProps) => {
  ...
  return (
    <div>
      <div
        className="flex-center flex items-center  justify-center p-2"
