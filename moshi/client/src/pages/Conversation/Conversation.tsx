...
import { type ModelParamsValues, useModelParams } from "./hooks/useModelParams";
import type { AudioStats } from "./hooks/useServerAudio";
...
const buildURL = ({
	...
	if (params.imageUrl != undefined) {
		...
		url.searchParams.append(
			"image_resolution",
			params.imageResolution.toString(),
		);
	}
	...
	const startRecording = useCallback(() => {
		...
		if (canvasRef.current) {
			...
			captureStream.addTrack(
				audioStreamDestination.current.stream.getAudioTracks()[0],
			);
			mediaRecorder.current = new MediaRecorder(captureStream, {
				mimeType: getMimeType("video"),
			...
			mediaRecorder.current.onstop = async () => {
				...
				if (mimeType.includes("webm")) {
					blob = await fixWebmDuration(
						new Blob(videoChunks.current, { type: mimeType }),
					);
				} else {
			...
			>
				<div>
					<div className="main-grid h-screen max-h-screen w-screen p-4 max-w-96 md:max-w-screen-lg m-auto">
								...
								<Button
									onClick={() => {
										...
										localStorage.setItem(
											"textTemperature",
											modelParams.textTemperature.toString(),
										);
										localStorage.setItem(
											"textTopk",
											modelParams.textTopk.toString(),
										);
										localStorage.setItem(
											"audioTemperature",
											modelParams.audioTemperature.toString(),
										);
										localStorage.setItem(
											"audioTopk",
											modelParams.audioTopk.toString(),
										);
										localStorage.setItem(
											"padMult",
											modelParams.padMult.toString(),
										);
										localStorage.setItem(
											"repetitionPenalty",
											modelParams.repetitionPenalty.toString(),
										);
										localStorage.setItem(
											"repetitionPenaltyContext",
											modelParams.repetitionPenaltyContext.toString(),
										);
										localStorage.setItem(
											"imageResolution",
											modelParams.imageResolution.toString(),
										);
										localStorage.setItem(
											"isImageMode",
											(modelParams.imageUrl != undefined).toString(),
										);
						...
						<div
							className="relative player h-full max-h-full w-full justify-between gap-3 border-2 border-white md:p-12"
							style={{
						...
						>
							...
							<div
								className="pt-8 text-sm flex justify-center items-center flex-col download-links"
								style={{
									minHeight: 80,
									margin: -10,
									padding: 0,
								...
								{videoURL && getExtension("video") === "webm" && (
									<div>
										<a
											...
											target="_blank"
											rel="noreferrer"
											className="explain-links pt-2 text-center italic block"
										>
					...
					<div className="max-w-96 md:max-w-screen-lg p-4 m-auto text-center">
						...
						{!workerAuthId && (
							<ModelParams
								...
								isImageMode={params.imageUrl != undefined}
							/>
						)}
					</div>
