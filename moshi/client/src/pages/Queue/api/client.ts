...
export const getAPIClient = (url: string) => ({
  ...
  checkUser: async (sessionId: number, sessionAuthId: string) => {
    ...
    const response = await fetch(
      `${url}/check_user?session_id=${encodedSessionId}&session_auth_id=${encodedSessionAuthId}`,
    );
    if (!response.ok) {
      const errorText = await response.text();
      throw new APIError(errorText, response.status);
  ...
  addFeedback: async ({
    ...
    const response = await fetch(
      `${url}/user_feedback?worker_auth_id=${encodedWorkerAuthId}&session_id=${encodedSessionId}&session_auth_id=${encodedSessionAuthId}&feedback=${encodedFeedback}&timestamp=${encodedTimestamp}&email=${encodedEmail}`,
    );
    if (!response.ok) {
      const errorText = await response.text();
      throw new APIError(errorText, response.status);
